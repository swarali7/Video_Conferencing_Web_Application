...
let recStartTime;
let recElapsedTime;
let videoTheme = "neon"; // neon - dark - forest - ghost ...
...
function getHtmlElementsById() {
  ...
  attendanceBtn = getId("attendanceBtn");
  captionText = getId("captionText");
...
function setButtonsTitle() {
  ...
  // left buttons
  tippy(attendanceBtn,{
    content: "Download Attendance List",
...
function initPeer() {
    ...
    Swal.fire({
     ...
     // imageUrl: welcomeImg,
      ...
      if (result.isConfirmed) {
        ...
        let message = {
          email: "",
          subject: "Please join our video Video Chat Meeting",
  ...
  signalingSocket.on("addPeer", (config) => {
    ...
    if (useRTCDataChannel) {
        ...
        event.channel.onmessage = (msg) => {
          switch (event.channel.label) {
            case "video_chat_channel":
              ...
              break;
            case "video_file_sharing_channel":
      ...
      createCaptionDataChannel(peer_id);
      // ...
    } else {
      ...
      signalingSocket.on("captions",(data)=>{
        addCaptions(data.msg);
      })
  ...
  signalingSocket.on("removePeer", (config) => {
    ...
    delete captionDataChannels[peer_id];
...
/**
 * Set video theme neon - dark - forest - sky - ghost
 * @param {*} theme
 */
...
/**
 * Set caption button click event
 */
 function setCaptionBtn() {
...
function downloadattend() {
  ...
  a.href =
    ...
    encodeURIComponent(JSON.stringify(attendance, null, 1));
  a.download = getDataTimeString() + "-attend.txt";
...
function addCaptions(captions) {
  document.querySelector( '#captionText' ).removeAttribute('display');
  captionText.innerHTML = `${ captions }`
}
 function requestToggleCaptions() {
  ...
  for (let peer_id in captionDataChannels) {
    
      captionDataChannels[peer_id].send("tog:");
    
  }
  signalingSocket.emit("tog:")
}
...
function toggleSendCaptions() {
  if (sendingCaptions) {
    ...
    //captionText.fadeOut()
    recognition.stop();
    document.querySelector( '#captionText' ).setAttribute('display','none')
  } else {
    startSpeech();
  ...
  recognition.onresult = (event) => {
    ...
    for (let i = event.resultIndex, len = event.results.length; i < len; i++) {
      ...
      if (event.results[i].isFinal) {
        ...
        data = {
          msg: myPeerName +" : " + interimTranscript.substring(interimTranscript.length - charsToKeep),
          sender: myPeerName,
      ...
      if (useRTCDataChannel) {
        ...
        Object.keys(captionDataChannels).map((peerId) =>
          captionDataChannels[peerId].send(JSON.stringify(captionMsg))
        );
      } else {
        signalingSocket.emit("captions",data);}
      }
    }
  };
...
function createCaptionDataChannel(peer_id) {
  captionDataChannels[peer_id] = peerConnections[peer_id].createDataChannel(
    "video_caption_channel"
  );
}
function handleDataChannelCaption(dataMessages) {
  const cleanedMessage = dataMessages.slice(4);
  const dataType = dataMessages.substring(0, 4);
  switch (dataType) {
    case "tag:":
      toggleSendCaptions();
      break;
 ...
 */
 function setScreenSharingStatus(status) {
  captionBtn.className = status ? "fas fa-closed-captioning fa-xs" : "fas fa-closed-captioning fa-xs";
  ...
  if (isImageURL(incomingFileInfo.fileName)) {
    ...
    reader.onload = (e) => {
      Swal.fire({
        ...
        imageAlt: "video-file-img-download",
        showDenyButton: true,
...
function disableAllPeers(element) {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    imageAlt: "video-disable-" + element,
    imageUrl: confirmImg,
    title:
      element == "audio"
        ? "Mute everyone except yourself?"
...
function getAbout() {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    title: "<strong>WebRTC Made by group 54</strong>",
   
    html: `
    <br/>
    <div id="about"><b>open source</b> project on Video Conferencing Web Application</div>
    `,
    showClass: {
      popup: "animate__animated animate__fadeInDown",
    },
